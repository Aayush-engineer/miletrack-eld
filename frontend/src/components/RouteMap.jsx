  ...
  useEffect(() => {
    ...
    if (route.polyline?.length > 1) {
      const shadow = L.polyline(route.polyline, {
        color: '#0a1628', weight: 8, opacity: 0.1, smoothFactor: 1.5,
      }).addTo(map)
      const line = L.polyline(route.polyline, {
        color: '#2563eb', weight: 4.5, opacity: 0.88, smoothFactor: 1.5,
      }).addTo(map)
    ...
    for (const wp of route.waypoints || []) {
      ...
      const m = L.marker([wp.lat, wp.lng], {
        icon: pinIcon(cfg.color, cfg.emoji, 36),
        zIndexOffset: 1000,
      })
        .bindTooltip(tooltip(wp.name), {
          permanent: false, direction: 'right', opacity: 1, className: 'mt-tooltip',
        })
        .bindPopup(popup(cfg.color, cfg.label, cfg.emoji, cfg.badge, [['Location', wp.name]]), {
          maxWidth: 260, className: 'mt-popup',
        })
        .addTo(map)
    ...
    for (const stop of route.stops || []) {
      ...
      const m = L.marker([stop.lat, stop.lng], {
        icon: circleIcon(color, isFuel ? 13 : 15),
        zIndexOffset: 500,
      })
        .bindTooltip(tooltip(ttText), {
          permanent: false, direction: 'top', opacity: 1, className: 'mt-tooltip',
        })
        .bindPopup(popup(color, label, emoji,
          stop.arrival_time ? `Arrival: ${fmtTime(stop.arrival_time)}` : '',
          rows,
        ), { maxWidth: 240, className: 'mt-popup' })
        .addTo(map)
      layersRef.current.push(m)
    }
      ...
      <div style={{
        ...
        {[
          ...
          <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: '#374151', fontFamily: F }}>
              {emoji} {label}
            </span>
          </div>
        ))}
        <p style={{
          fontSize: 9, color: '#9ca3af', marginTop: 10,
          borderTop: '1px solid #f3f4f6', paddingTop: 8, fontFamily: FM,
        }}>
